...
TOKEN = '8829797918:AAGST7ZZMxyCdUOdCuIsQhx5N6wtsFMygFw'  # Your bot token
ADMIN_ID = 5857450122  # Your admin ID
YOUR_USERNAME = '@R4GE143'  # Your username with @

bot = telebot.TeleBot('8829797918:AAGST7ZZMxyCdUOdCuIsQhx5N6wtsFMygFw')
...
def broadcast_callback(call):
    if call.from_user.id == ADMIN_ID:
        ...
        bot.send_message(call.message.chat.id, "⚠️ You are not the developer.")
    ...
    try:
        ...
        if token:
            try:
                ...
                caption = f"📤 User {user_info} uploaded a new bot file, but couldn't fetch bot username."
        else:
        ...
        bot.send_message(chat_id, f"❌ Error running the bot: {e}")
...
def get_custom_file_to_run(message):
    try:
        ...
        if os.path.exists(custom_file_path):
            ...
            bot.send_message(chat_id, f"❌ The specified file doesn't exist. Please check the name and try again.")
    except Exception as e:
        bot.send_message(message.chat.id, f"❌ Error: {e}")
